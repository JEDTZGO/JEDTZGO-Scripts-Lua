...
Entregable single-file compatible con loadstring.

Migrado desde archive/ProjectDelta_v5_legacy.lua.
Organizado por secciones MODULE sin dividir el script en archivos externos.
]]
...
-- Project Delta v5.3 legacy organizado para main.lua single-file.
...
-- =========================================================
-- MODULE: FEATURE / AIM PREDICTIVO BETA
-- =========================================================
...
task.spawn(function()
    ...
    local lEtiq=mkTx(0,0,"JUGADORES",C_ACCENT,11,8); local lFondo=mkSq(0,0,LP.a,20,C_PANEL_INNER,8,1,3)
    ...
    local rEtiq=mkTx(0,0,"INVENTARIO",C_DIM,11,10); local rTitulo=mkTx(0,0,"",C_ACCENT,12,10)
    ...
    UI.AddTab("Project Delta",function(tab)
        ...
        secMundo:Toggle("cuerpoEsp","ESP de cadaveres",CFG.cuerpoEsp,function(estado) CFG.cuerpoEsp=estado; guardarCFGActual() end)
        ...
        secCombate:Toggle("noRetroceso","No retroceso",CFG.noRetroceso,function(estado)
            ...
            if estado then
                ...
                if restaurados>0 and fallidos==0 then
                    notify("No retroceso restaurado en esta sesion","Project Delta",3)
                elseif restaurados>0 then
                    notify("Restauracion parcial; reinicia partida/script si notas recoil raro","Project Delta",4)
                else
                    notify("No hay restauracion segura completa; reinicia partida/script si ya estaba aplicado","Project Delta",4)
                end
            end
            guardarCFGActual()
        end)
        secCombate:Toggle("noSpread","Sin dispersion",CFG.noSpread,function(estado)
            CFG.noSpread=estado
            if estado then
                recapturarNoSpread(true)
                notify("Sin dispersion activado","Project Delta",3)
            else
                desactivarNoSpread()
                notify("Sin dispersion desactivado","Project Delta",3)
            end
            guardarCFGActual()
        end)
        secCombate:Spacing()
        secCombate:Toggle("predictionAim","Aim predictivo BETA",CFG.predictionAim,function(estado)
            CFG.predictionAim=estado
            PRED_KEY_TOGGLE=false
            notify(estado and "Aim predictivo BETA activado" or "Aim predictivo BETA desactivado","Project Delta",3)
            guardarCFGActual()
        end)
        ...
        secCombate:Combo("predictionAimModo","Modo tecla",{"Alternar","Mantener","Siempre"},CFG.predictionAimModo or 0,function(indice)
        ...
        local secTema=tab:Section("Tema","Right")
        ...
        if type(secTema.ColorPicker)=="function" then
            pcall(function()
                secTema:ColorPicker("accentColor","Color accent",C_ACCENT,function(col)
        ...
        secTema:Toggle("panelStripes","Rayas de panel",CFG.panelStripes,function(estado)
    ...
    task.spawn(function()
        while true do
            task.wait(0.5)
            local activos={}
            -- ranuras se conserva porque el ESP de cadaveres reutiliza etiquetas Drawing.
