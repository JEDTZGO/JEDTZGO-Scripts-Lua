...
-- ─── Colores ESP (etiquetas en el mundo) ───────────────────────────────────────
...
-- CFG es la única fuente de verdad para todos los ajustes.
...
-- ─── Serialización (Lua puro, sin APIs del executor) ───────────────────────────
-- Formato: "PD|clave:valor|clave:valor|..."  — compacto, se puede pegar en una sola línea.
local function exportCFG()
    ...
    for k,v in pairs(CFG) do
        ...
        elseif t=="number" then
            -- redondea a 4 decimales para mantener el string corto
...
-- Intenta setclipboard (disponible en la mayoría de executors). Si no existe, lo ignora.
...
-- Usa CFG directamente — seguro de llamar en cualquier momento, incluso antes de UI.AddTab
...
-- ─── Panel derecho (inventario, Drawing) ────────────────────────────────────────
...
local rDLbl    = mkTx (0,0,":: arrastra", C_DIM,11,10)
...
local rEmpty   = mkTx(0,0,"  (vacío)",C_DIM,12,11)
...
local function showRight(title, items)
    ...
    if n==0 then
        ...
        if ext>0 then
            rMore.Size=fs; rMore.Text="  + "..ext.." más"
...
UI.AddTab("Project Delta", function(tab)
    ...
    invSec:Toggle("showList", "Lista de jugadores", CFG.showList, function(state)
        CFG.showList=state
        if CFG.invEsp then
    ...
    invSec:Toggle("aimOn", "Panel de mira", CFG.aimOn, function(state)
        CFG.aimOn=state
    ...
    opSec:SliderFloat("listOpacity", "Panel lista", 0.0, 1.0, CFG.listOpacity, "%.2f", function(val)
        CFG.listOpacity=val
        lBg.Transparency=1-val; lDrag.Transparency=1-val
    end)

    opSec:SliderFloat("rightOpacity", "Inv / Panel mira", 0.0, 1.0, CFG.rightOpacity, "%.2f", function(val)
        CFG.rightOpacity=val
        rBg.Transparency=1-val; rDrag.Transparency=1-val
    end)

    -- Derecha: Guardar / Cargar config
    local saveSec = tab:Section("Config", "Right")

    saveSec:Text("Exportar copia tu config al portapapeles.")
    saveSec:Text("Pégala en el campo Importar para restaurarla.")
    saveSec:Spacing()

    -- Exportar: codifica el CFG actual + posiciones y lo copia al portapapeles
    saveSec:Button("Exportar config  (copiar al portapapeles)", function()
        -- sincroniza posiciones de paneles en CFG antes de exportar
        CFG.lx=INV.lx; CFG.ly=INV.ly
        CFG.rx=INV.rx; CFG.ry=INV.ry
        local str = exportCFG()
        copyToClipboard(str)
        notify("¡Config copiada al portapapeles!", "Project Delta", 3)
    end)
    ...
    local importField = saveSec:InputText("cfgImport", "Pegar config aquí", "", function(text)
        if importCFG(text) then
            ...
            -- restaura posiciones de paneles
            ...
            notify("¡Config cargada!", "Project Delta", 3)
        else
            notify("String de config inválido.", "Project Delta", 3)
        end
    end)
    ...
    miscSec:Button("Resetear a valores por defecto", function()
        for k,v in pairs(CFG_DEFAULTS) do CFG[k]=v end
        UI.SetValue("invEsp",       CFG.invEsp)
        UI.SetValue("showList",     CFG.showList)
        UI.SetValue("aimOn",        CFG.aimOn)
...
-- ─── Drag del panel de lista + navegación por teclado ───────────────────────────
...
spawn(function()
    ...
    while true do
        ...
        -- renderizado de etiquetas ESP en el mundo
...
-- ─── Panel aim: seguimiento al jugador más cercano al crosshair ────────────────
lastAimName=nil
spawn(function()
    while true do
        task.wait(0.15)
...
-- ─── Escáner ESP de NPCs y cadáveres ────────────────────────────────────────────
spawn(function()
    while true do
        task.wait(0.5)
        local playerNames={}
...
-- ─── Observador de conteo de jugadores ──────────────────────────────────────────
