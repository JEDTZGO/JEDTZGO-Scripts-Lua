...
local K_HOME=112;local K_UP=38;local K_DOWN=40;local K_ENTER=13;local K_BKSP=8
...
local t2Lbl=tx(0,0,"lista jugadores",C_TEXT,12,22);local t2Bg=sq(0,0,36,18,C_ON,23,1,9);local t2Dot=sq(0,0,14,14,C_THUMB,24,1,7)
...
local wSep2=ln(0,0,0,0,C_BORDER,0.3,21);local sSec3=tx(0,0,"opacidad",C_DIM,10,21)
local sl1Lbl=tx(0,0,"lista",C_TEXT,12,22);local sl1Track=sq(0,0,120,4,C_OFF,22,1,2);local sl1Thumb=sq(0,0,10,10,C_ACCENT,23,1,5);local sl1Val=tx(0,0,"85%",C_DIM,11,22)
...
local lDLbl=tx(0,0,":: jugadores",C_ACCENT,11,8);local lBg=sq(0,0,LP.w,20,C_BG,8,1,3)
...
local rDLbl=tx(0,0,":: arrastra",C_DIM,11,10);local rTitle=tx(0,0,"",C_ACCENT,12,10)
...
local rEmpty=tx(0,0,"  (vacio)",C_DIM,12,11);local rMore=tx(0,0,"",C_DIM,12,11)
...
local function showRight(title,items)
    ...
    if n==0 then
        ...
        if ext>0 then
            rMore.Size=fs;rMore.Text="  + "..ext.." mas"
...
notify("Project Delta  -  F1 para configuracion","PD v4.0",4)
